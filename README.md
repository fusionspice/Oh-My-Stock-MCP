...
삼성증권과 미래에셋증권의 로그인 세션을 사용해 종목 단위 보유내역만 JSON으로 출력하는 명령입니다. 주문·정정·취소 API는 호출하지 않습니다.

```bash
npm run export:holdings -- --brokers samsungpop,miraeasset
```

출력에는 계좌번호, 종목코드, 수량, 매입단가, 현재가 등 보유현황에 필요한 필드만 포함되며 원본 페이지·쿠키·비밀번호는 포함되지 않습니다.
